...
const int SENSOR_LEFT      = 38;
const int SENSOR_CENTER    = 34;
...
void controlMotors(float pidOutput) {
 ...
 // Set directions
 digitalWrite(MOTOR_LEFT_DIR, leftSpeed >= 0 ? FORWARD : BACKWARD);
 digitalWrite(MOTOR_RIGHT_DIR, rightSpeed >= 0 ? FORWARD : BACKWARD);
 delayMicroseconds(100);
...
void controlTask(void *pvParameters) {
 ...
 while (true) {
   ...
   if (now - lastPIDTime >= sampleTime) {
     ...
     if (!sLL && !sL && !sC && !sR && !sRR) {
         float lastLL = lastPID[0];
         float lastRR = lastPID[4];
